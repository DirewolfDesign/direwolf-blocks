...
/**
 * DirewolfBlocks_Tools class. Class to work with DirewolfBlocks CPT.
 */
class DirewolfBlocks_Tools {
    /**
     * Admin notices
     *
     * @var array
    ...
    /**
     * DirewolfBlocks_Tools constructor.
     */
    ...
    public function admin_menu() {
        $page = add_submenu_page(
            ...
            esc_html__( 'Export / Import', 'dwb' ),
            esc_html__( 'Export / Import', 'dwb' ),
            'manage_options',
            'direwolfblocks_tools',
    ...
    public function render_tools_page() {
        if( file_exists( dwb()->plugin_path() . '/components/admin/dwb-tools-page.php' ) )
            include( dwb()->plugin_path() . '/components/admin/dwb-tools-page.php' );
    }
    ...
    public function clean_block_to_export( $block ) {
        $db_blocks = dwb()->blocks()->get_blocks( true );
    ...
    public function admin_enqueue_scripts() {
        ...
        if ( 'direwolfblocks_page_direwolfblocks_tools' !== $screen->id ) {
            return;
        }

        $blocks    = dwb()->blocks()->get_blocks( true, true, true );
        $templates = dwb()->templates()->get_templates( true, true );
        $data      = array(
            'blocks'    => array(),
            'templates' => array(),
            'nonce'     => wp_create_nonce( 'dwb-tools-import-nonce' ),
        );
        ...
        wp_enqueue_script(
            'direwolfblocks-tools',
            dwb()->plugin_url() . 'assets/admin/tools/index.min.js',
            array( 'wp-data', 'wp-element', 'wp-components', 'wp-api', 'wp-i18n' ),
            '2.3.0',
            true
        );

        wp_localize_script( 'direwolfblocks-tools', 'direwolfblocksToolsData', $data );
    }
    ...
    public function import_json() {
        ...
        // phpcs:ignore
        $nonce = isset( $_POST[ 'dwb_tools_import_nonce' ] ) ? $_POST[ 'dwb_tools_import_nonce' ] : false;

        if ( ! $nonce || ! wp_verify_nonce( $nonce, 'dwb-tools-import-nonce' ) ) {
            return;
        }

        // Check file size.
        if ( empty( $_FILES['dwb_tools_import_json']['size'] ) ) {
            return $this->add_notice( esc_html__( 'No file selected', 'dwb' ), 'warning' );
        }

        // Get file data.
        // phpcs:ignore
        $file = $_FILES['dwb_tools_import_json'];

        // Check for errors.
        if ( $file['error'] ) {
            return $this->add_notice( esc_html__( 'Error uploading file. Please try again', 'dwb' ), 'warning' );
        }

        // Check file type.
        if ( pathinfo( $file['name'], PATHINFO_EXTENSION ) !== 'json' ) {
            return $this->add_notice( esc_html__( 'Incorrect file type', 'dwb' ), 'warning' );
        }
        ...
        if ( ! $json || ! is_array( $json ) ) {
            return $this->add_notice( esc_html__( 'Import file empty', 'dwb' ), 'warning' );
        }
        ...
        if ( ! empty( $imported_blocks ) ) {
            ...
            $text = sprintf( esc_html( _n( 'Imported %s block', 'Imported %s blocks', $total_blocks, 'dwb' ) ), $total_blocks );
        ...
        if ( ! empty( $imported_templates ) ) {
            ...
            $text = sprintf( esc_html( _n( 'Imported %s template', 'Imported %s templates', $total_templates, 'dwb' ) ), $total_templates );
    ...
    public function maybe_export_json() {
        $block_id  = filter_input( INPUT_GET, 'direwolfblocks_export_block', FILTER_SANITIZE_NUMBER_INT );
        $block_ids = filter_input_array(
            INPUT_GET,
            array(
                'direwolfblocks_export_blocks' => array(
                    'filter' => FILTER_SANITIZE_NUMBER_INT,
                    'flags'  => FILTER_REQUIRE_ARRAY,
                ),
            )
        );
        $block_ids = is_array( $block_ids ) && isset( $block_ids['direwolfblocks_export_blocks'] ) ? $block_ids['direwolfblocks_export_blocks'] : array();

        $template_ids = filter_input_array(
            INPUT_GET,
            array(
                'direwolfblocks_export_templates' => array(
                    'filter' => FILTER_SANITIZE_NUMBER_INT,
                    'flags'  => FILTER_REQUIRE_ARRAY,
                ),
            )
        );
        $template_ids = is_array( $template_ids ) && isset( $template_ids['direwolfblocks_export_templates'] ) ? $template_ids['direwolfblocks_export_templates'] : array();
    ...
    public function export_json( $items, $type = 'blocks' ) {
        ...
        if ( 'blocks' === $type ) {
            $blocks = dwb()->blocks()->get_blocks( true );
            ...
            $templates = dwb()->templates()->get_templates( true );
        ...
        header( 'Content-disposition: attachment; filename=dwb-export-' . $type . '-' . date_i18n( 'Y-m-d' ) . '.json' );
    ...
    private function import_block( $data ) {
        ...
        if ( 0 < $post_id ) {
            ...
            dwb()->blocks()->save_meta_boxes( $post_id, $meta );

            do_action( 'dwb/import/block', $post_id, $data );
    ...
    private function import_template( $data ) {
        ...
        foreach ( $templates as $template ) {
            if ( $template['post_type']['data']['post_type'] === $data['post_type'] ) {
                ...
                $text = sprintf( esc_html__( 'Template for post type \'%s\' already exists.', 'dwb' ), $data['post_type'] );
        ...
        if ( 0 < $post_id ) {
            ...
            // phpcs:ignore
            add_post_meta( $post_id, 'dwb_template_data', urlencode( json_encode( $template_data ) ) );

            do_action( 'dwb/import/template', $post_id, $template_data );
    ...
    public function maybe_duplicate_block() {
        $block_id          = filter_input( INPUT_GET, 'direwolfblocks_duplicate_block', FILTER_SANITIZE_NUMBER_INT );
        $block_id_complete = filter_input( INPUT_GET, 'direwolfblocks_duplicate_complete', FILTER_SANITIZE_NUMBER_INT );
        ...
        if ( isset( $block_id_complete ) && $block_id_complete ) {
            ...
            if ( isset( $post ) && $post ) {
                ...
                $text = sprintf( esc_html__( 'Added new block \'%s\'.', 'dwb' ), $post->post_title );
                $this->add_notice( $text, 'success' );
            }
        }
    }
    ...
    public function duplicate_block( $block_id ) {
        // Check for nonce security.
        // phpcs:ignore
        $nonce = isset( $_GET[ 'direwolfblocks_duplicate_block_nonce' ] ) ? $_GET[ 'direwolfblocks_duplicate_block_nonce' ] : false;

        if ( ! $nonce || ! wp_verify_nonce( $nonce, 'dwb-duplicate-block-nonce' ) ) {
            return;
        }
        ...
        if ( isset( $post ) && $post ) {
            ...
            // Redirect.
            wp_safe_redirect( admin_url( 'edit.php?post_type=direwolfblocks&direwolfblocks_duplicate_complete=' . $new_post_id ) );
