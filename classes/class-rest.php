...
/**
 * Class DirewolfBlocks_Rest
 */
class DirewolfBlocks_Rest extends WP_REST_Controller {
    /**
     * Namespace.
     *
     * @var string
     */
    protected $namespace = 'direwolfblocks/v';
    ...
    /**
     * DirewolfBlocks_Rest constructor.
     */
    ...
    public function get_block_permission( $request ) {
        ...
        if ( 0 < $post_id ) {
            ...
            if ( ! $post || ! current_user_can( 'edit_post', $post->ID ) ) {
                return $this->error( 'dwb_block_cannot_read', esc_html__( 'Sorry, you are not allowed to read Gutenberg blocks of this post.', 'dwb' ) );
            }
        } else {
            if ( ! current_user_can( 'edit_posts' ) ) {
                return $this->error( 'dwb_block_cannot_read', esc_html__( 'Sorry, you are not allowed to read Gutenberg blocks as this user.', 'dwb' ) );
            }
        }
    ...
    public function get_block_data_permission() {
        ...
        if ( 0 < $post_id ) {
            ...
            $post = get_post( $post_id );
            if ( ! $post || ! current_user_can( 'edit_post', $post->ID ) ) {
                return $this->error( 'dwb_block_data_cannot_read', esc_html__( 'Sorry, you are not allowed to read Gutenberg blocks data.', 'dwb' ) );
            }
        } else {
            if ( ! current_user_can( 'edit_posts' ) ) {
                return $this->error( 'dwb_block_data_cannot_read', esc_html__( 'Sorry, you are not allowed to read Gutenberg blocks data as this user.', 'dwb' ) );
            }
        }
    ...
    public function get_block( $request ) {
        ...
        $block = dwb()->blocks()->get_block( $block_name );

        if ( ! $block ) {
            return $this->error( 'dwb_block_invalid', esc_html__( 'Invalid block.', 'dwb' ) );
        }
        ...
        $block_result = dwb()->blocks()->render_callback( $block_attributes, null, $block_context );

        if ( isset( $block_result ) && null !== $block_result ) {
            return $this->success( $block_result );
        } else {
            return $this->error( 'dwb_block_no_render_callback', esc_html__( 'Render callback is not specified.', 'dwb' ) );
        }
    }
    ...
    public function get_block_data( $request ) {
        ...
        if ( 0 < $post_id ) {
            $post_meta   = dwb()->blocks()->get_meta_boxes( $post_id );
            $meta_prefix = 'dwb_';
